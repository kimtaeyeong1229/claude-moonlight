...
- **`wp` (단어쌍, 모든 문장 필수)**: 그 문장 안의 단어/구에 대한 영↔한 대응 목록 `[[영어, 한국어], ...]`.
  - 사용자가 문장 위에서 단어를 더블클릭하면, 그 단어와 한국어 대응어가 양쪽에서 더 밝게 강조됨(단어 뜻 학습용).
  - **모든 문장(`ss` 의 모든 항목)에 `wp` 를 넣으세요.** 더블클릭했을 때 짝이 안 뜨는 문장이 없도록,
    그 문장의 **내용어(명사·전문용어·동사·형용사)를 거의 빠짐없이** 담습니다(관사·전치사 등 기능어만 제외).
  - 영어 `en` 은 문장에 **실제로 등장하는 표기 그대로**(대소문자 무관 매칭). 복합어/구는 구 단위로(예: `["dense attention","밀집 어텐션"]`).
  - 한국어 쪽은 **조사 없이 깔끔한 형태**(예: "어텐션", "메모리").
  - 짧은 문장이라도 최소 2~3개는 채우고, 단어쌍을 만들 내용어가 정말 없을 때만 빈 배열로 둡니다.
